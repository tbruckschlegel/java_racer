...
public class JmeBinaryWriter {
     ...
     */
    private IdentityHashMap sharedObjects=new IdentityHashMap();
     ...
     */
    private IdentityHashMap entireScene=new IdentityHashMap();
    ...
    private void writeTerrainPage(TerrainPage terrainPage) throws IOException {
        ...
        atts.put("totsize",Integer.valueOf(terrainPage.getTotalSize()));
        atts.put("size",Integer.valueOf(terrainPage.getSize()));
        atts.put("stepscale",terrainPage.getStepScale());
        atts.put("offamnt",Integer.valueOf(terrainPage.getOffsetAmount()));
    ...
    private void writeTerrainBlock(TerrainBlock terrainBlock) throws IOException {
        ...
        atts.put("trisppix", Float.valueOf(terrainBlock.getTrisPerPixel()));
        atts.put("disttol",Float.valueOf(terrainBlock.getDistanceTolerance()));

        atts.put("tbsize",Integer.valueOf(terrainBlock.getSize()));
        atts.put("totsize",Integer.valueOf(terrainBlock.getTotalSize()));
        atts.put("step",terrainBlock.getStepScale());
        atts.put("isclod", Boolean.valueOf(terrainBlock.isUseClod()));
        atts.put("offset",terrainBlock.getOffset());
        atts.put("offamnt",Integer.valueOf(terrainBlock.getOffsetAmount()));
    ...
    private void writeAreaClod(AreaClodMesh areaClodMesh) throws IOException {
        ...
        atts.put("trisppix",Float.valueOf(areaClodMesh.getTrisPerPixel()));
        atts.put("disttol",Float.valueOf(areaClodMesh.getDistanceTolerance()));
    ...
    private void writeRecords(CollapseRecord[] records) throws IOException {
        ...
        HashMap atts=new HashMap();
        atts.clear();
        atts.put("numrec",Integer.valueOf(records.length));
        writeTag("clodrecords",atts);
        for (int i=0;i<records.length;i++){
            atts.clear();
            atts.put("index",Integer.valueOf(i));
            atts.put("numi",Integer.valueOf(records[i].numbIndices));
            atts.put("numt",Integer.valueOf(records[i].numbTriangles));
            atts.put("numv",Integer.valueOf(records[i].numbVerts));
            atts.put("vkeep",Integer.valueOf(records[i].vertToKeep));
            atts.put("vthrow",Integer.valueOf(records[i].vertToThrow));
    ...
    private void addControllerBaseAtts(Controller c,HashMap atts){
        atts.put("rptype",Integer.valueOf(c.getRepeatType()));
        atts.put("speed",Float.valueOf(c.getSpeed()));
    }
    ...
    private void writeSpatialTransformer(SpatialTransformer st) throws IOException {
        ...
        atts.put("numobjects",Integer.valueOf(st.getNumObjects()));
        writeTag("spatialtransformer",atts);
        for (int i=0;i<st.toChange.length;i++){
            atts.clear();
            atts.put("obnum",Integer.valueOf(i));
            atts.put("parnum",Integer.valueOf(st.parentIndexes[i]));
            writeTag("stobj",atts);
    ...
    private void writeSpatialTransformerPointInTime(SpatialTransformer.PointInTime pointInTime) throws IOException {
        HashMap atts=new HashMap();
        atts.clear();
        atts.put("time",Float.valueOf(pointInTime.time));
    ...
    private void writeTriMeshTags(TriMesh triMesh) throws IOException {
        ...
        atts.clear();
        for (int i=0;i<triMesh.getAllTextures().length;i++){
            if (triMesh.getTextures(i).length>0) {
                if (i!=0)
                    atts.put("texindex",Integer.valueOf(i));
    ...
    private void writeBoundingSphere(BoundingSphere v) throws IOException {
        ...
        if (sharedObjects.containsKey(v))
            atts.put("sharedident",sharedObjects.get(v));
        atts.put("center",v.getCenter());
        atts.put("radius",Float.valueOf(v.getRadius()));
    ...
    private void writeJointController(JointController jc) throws IOException{
        ...
        atts.put("numJoints",Integer.valueOf(jc.numJoints));
        atts.put("fps",Float.valueOf(jc.FPS));
...
//        Quaternion tempQ=new Quaternion();
        ...
        for (int j=0;j<jc.numJoints;j++){
            atts.clear();
            atts.put("index",Integer.valueOf(j));
            atts.put("parentindex",Integer.valueOf(jc.parentIndex[j]));
            ...
            for (int i=0;i<o.length;i++){
                ...
                if (jp.usedTrans.get(j) || jp.usedRot.get(j)){
                    atts.clear();
                    atts.put("time",Float.valueOf(jp.time));
    ...
    private void writeWireframeState(WireframeState wireframeState) throws IOException {
        ...
        atts.put("width",Float.valueOf(wireframeState.getLineWidth()));
        atts.put("facetype",Integer.valueOf(wireframeState.getFace()));
    ...
    private void putLightProperties(Light child, HashMap atts) {
        ...
        atts.put("fconstant",Float.valueOf(child.getConstant()));
        atts.put("diffuse",child.getDiffuse());
        atts.put("flinear",Float.valueOf(child.getLinear()));
        atts.put("fquadratic",Float.valueOf(child.getQuadratic()));
        atts.put("specular",child.getSpecular());
        atts.put("isattenuate",Boolean.valueOf(child.isAttenuate()));
    }

    private void writeSpotLight(SpotLight spotLight,HashMap atts) throws IOException {
        atts.put("loc",spotLight.getLocation());
        atts.put("fangle",Float.valueOf(spotLight.getAngle()));
        atts.put("dir",spotLight.getDirection());
        atts.put("fexponent",Float.valueOf(spotLight.getExponent()));
    ...
    private void writeTextureState(TextureState state) throws IOException{
        ...
        for (int i=0;i<state.getNumberOfUnits();i++){
            ...
            {                
                ...
                String texFile=null;
                char[] tmpBuffer = replaceSpecialsForFile(new StringBuilder(s.substring(6))).toString().toCharArray();
                ...
                if(texFile!=null)
                {
                    //System.out.println("new txt: "+texFile);
                    atts.put("file", texFile);
                }
                else
                {
                    //System.out.println("or. txt: "+replaceSpecialsForFile(new StringBuilder(s.substring(6))).toString());
                    atts.put("file",replaceSpecialsForFile(new StringBuilder(s.substring(6))).toString());
                }
                    
            }
            else
                atts.put("URL",new URL(s));
            atts.put("wrap",Integer.valueOf(toTest.getWrap()));
            atts.put("texnum",Integer.valueOf(i));
     ...
     */
    private static StringBuilder replaceSpecialsForFile(StringBuilder s) {
        int i=s.indexOf("%20");
    ...
    private void writeMaterialState(MaterialState state) throws IOException {
        ...
        atts.put("alpha",Float.valueOf(state.getAlpha()));
        atts.put("shiny",Float.valueOf(state.getShininess()));
